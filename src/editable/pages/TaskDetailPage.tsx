...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-white">
      <header className="editable-tech-bg">
        <div className="mx-auto max-w-[1180px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 text-[11px] font-black uppercase tracking-[0.16em]">
          ...
          <h1 className="mt-6 max-w-6xl text-5xl font-black leading-tight sm:text-6xl lg:text-[5rem]">{post.title}</h1>
          {!isMediaDistribution && summaryText(post) ? <p className="mt-6 max-w-4xl text-xl font-bold leading-8 text-black/68 sm:text-2xl">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[1280px] px-4 pt-10 sm:px-6 lg:px-8">
          <img src={images[0]} alt="" className="max-h-[760px] w-full rounded-[2rem] object-cover shadow-sm" />
          {!isMediaDistribution ? <figcaption className="px-4 py-3 text-xs italic text-black/55 sm:px-6">Featured image for {post.title}</figcaption> : null}
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1280px] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,1fr)_330px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-[2rem] bg-[#f2f5f7] p-6 sm:p-8">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div>
          <RelatedPanel task={task} post={post} related={related} hideOverview={isMediaDistribution} />
        </div>
      </div>
    </section>
  )
...
function RelatedPanel({ task, post, related, compact = false, hideOverview = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean; hideOverview?: boolean }) {
  const taskConfig = getTaskConfig(task)
  return (
    <aside className="min-w-0 space-y-5">
      {!compact && !hideOverview ? (
        <div className="rounded-[2rem] bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 text-sm font-bold opacity-75">
            ...
            {task !== 'mediaDistribution' && post.publishedAt ? <p>Published: {new Date(post.publishedAt).toLocaleDateString()}</p> : null}
          </div>
        </div>
      ) : null}
      {related.length ? (
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-black/10 py-3 transition hover:text-[var(--slot4-accent)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-2xl object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-black text-white"><FileText className="h-6 w-6" /></div>}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-[2rem] bg-white p-5 shadow-sm">
