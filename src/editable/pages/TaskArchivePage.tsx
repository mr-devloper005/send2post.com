...
const getPostType = (post: SitePost) => asText(getContent(post).type)
...
async function fetchRealMediaDistributionPosts(page: number, limit: number, category: string) {
  const feed = await fetchSiteFeed<SitePost>(100, { fresh: true, timeoutMs: 5000 }).catch(() => null)
  const filtered = (feed?.posts || []).filter((post) => {
    const postType = getPostType(post)
    if (postType && postType !== 'mediaDistribution') return false
    const postCategory = normalizeCategory(getCategory(post, ''))
...
export async function EditableTaskArchiveRoute({
  ...
  const result = await fetchPaginatedTaskPosts(task, { page, limit, category })
  const recovered = task === 'mediaDistribution' && !result.posts.length
    ? await fetchRealMediaDistributionPosts(page, limit, category)
    : null
  const visible = recovered || result
  return <TaskArchiveView task={task} posts={visible.posts} pagination={visible.pagination} category={category} basePath={basePath || taskConfig?.route || `/${task}`} />
}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[#111]">
        <section className="editable-tech-bg bg-white">
          <div className="mx-auto flex max-w-[1280px] flex-col gap-6 px-4 py-14 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-20">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-accent)]">Media distribution archive</p>
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-black/65">
              Timely releases, campaign notes, and media-ready stories organized for fast discovery.
            </p>
          </div>
        </section>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1280px] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[1.65fr_0.75fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden rounded-[2rem] bg-black shadow-sm">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em]">{getCategory(lead, label)}</span>
            ...
            <div className="grid gap-5">
              <div className="rounded-[2rem] bg-[var(--slot4-accent)] p-6 text-white shadow-sm">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight">What the distribution desk is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] overflow-hidden rounded-[2rem] bg-[#f2f5f7] shadow-sm">
        ...
        <section className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5">
            ...
            <form action={basePath} className="flex rounded-full border border-black/10 bg-white p-1 shadow-sm">
              ...
              <button className="h-11 rounded-full bg-black px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {cardPosts.length ? (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {cardPosts.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-[2rem] bg-[#f2f5f7] shadow-sm">
            ...
            <div className="rounded-[2rem] border border-dashed border-black/20 bg-white p-12 text-center shadow-sm">
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-black/10 bg-white px-5 py-3 text-xs font-black uppercase shadow-sm">Previous</Link> : null}
            <span className="rounded-full bg-[var(--slot4-accent)] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-black/10 bg-white px-5 py-3 text-xs font-black uppercase shadow-sm">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
