...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] bg-[#f2f5f7] shadow-sm ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute left-4 top-4 rounded-full bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-full bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span> : null}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-black">
        <section className="editable-tech-bg">
          <div className="mx-auto grid max-w-[1280px] gap-8 px-4 py-14 sm:px-6 md:grid-cols-[0.8fr_1.2fr] lg:px-8">
            <div>
              <div className="h-full rounded-[2.5rem] bg-[var(--slot4-blue)] p-7 text-white sm:p-10 lg:p-14">
            ...
            <form action="/search" className="self-center rounded-[2.5rem] bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] sm:p-10 lg:p-14">
              <input type="hidden" name="master" value="1" />
              <label className="flex items-center gap-3 rounded-full border border-black/10 bg-white px-4 py-3 shadow-sm">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="flex items-center gap-2 rounded-full border border-black/10 bg-white px-4 py-3 shadow-sm">
                ...
                <select name="task" defaultValue={task} className="rounded-full border border-black/10 bg-white px-4 py-3 text-sm font-black outline-none shadow-sm">
                  <option value="">All content types</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--slot4-dark-bg)] px-6 text-xs font-black text-white transition hover:bg-[var(--slot4-accent)]" type="submit">Search</button>
            </form>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-[1280px] px-4 py-14 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/media-distribution" className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white px-5 py-3 text-xs font-black shadow-sm">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-black/20 bg-white p-10 text-center shadow-sm">
