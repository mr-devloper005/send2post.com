...
export function EditorialFeatureCard({ post, href, label = 'Cover story' }: { post: SitePost; href: string; label?: string }) {
  return (
    <Link href={href} className="group block min-w-0 overflow-hidden rounded-[2rem] bg-black text-white">
      <div className="relative aspect-[16/10] min-h-[430px] overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 p-6 sm:p-9">
          <span className="rounded-full bg-[var(--slot4-accent-fill)] px-3 py-2 text-[10px] font-black uppercase tracking-[.18em]">{label}</span>
...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden rounded-[2rem] bg-[#f2f5f7] shadow-sm ${dc.motion.lift}`}>
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 grid-cols-[46px_1fr] gap-4 rounded-[1.5rem] border border-black/10 bg-white p-4 shadow-sm">
...
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 overflow-hidden rounded-[2rem] bg-[#f2f5f7] shadow-sm sm:grid-cols-[240px_minmax(0,1fr)]">
